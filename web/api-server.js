...
// 生成 Word 文档（带目录和样式）
async function generateWordDocument(project, requirements, testcases, traces, options = {}) {
    const { includeRequirements = true, includeTestcases = true, includeTraceMatrix = true, groupByHeader = true } = options;
    ...
    if (includeRequirements && requirements.length > 0) {
        ...
        if (groupByHeader) {
            ...
            requirements.forEach(req => {
                const headerId = req.headerId || 'root';
                if (!headersMap.has(headerId)) {
                    headersMap.set(headerId, []);
                }
                headersMap.get(headerId).push(req);
            });
            
            for (const [headerId, reqs] of headersMap) {
                if (headerId !== 'root') {
                ...
                reqs.forEach((req, index) => {
                    children.push(
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `${req.reqId} - ${req.title || req.description?.substring(0, 50) || '未命名'}`,
                                    bold: true,
                                    size: 24
                                })
                            ],
                            ...
                            rows: [
                                new TableRow({
                                    children: [
                                        ...
                                        new TableCell({
                                            children: [new Paragraph(req.category || '-')],
                                ...
                                new TableRow({
                                    children: [
                                        new TableCell({
                                            children: [new Paragraph('优先级')],
                                            shading: { fill: 'f5f5f5' }
                                        }),
                                        new TableCell({
                                            children: [new Paragraph(req.priority || '-')]
                                        })
                                    ]
                                }),
                                new TableRow({
                                    children: [
                                        ...
                                        new TableCell({
                                            children: [new Paragraph(req.assignee || '-')]
                                        })
                                    ]
                                }),
                                new TableRow({
                                    children: [
                                        ...
                                        new TableCell({
                                            children: [new Paragraph(stripHtml(req.description || '无'))]
                                        })
                                    ]
                                }),
                                new TableRow({
                                    children: [
                                        new TableCell({
                                            children: [new Paragraph('验收标准')],
                                            shading: { fill: 'f5f5f5' }
                                        }),
                                        new TableCell({
                                            children: [new Paragraph(stripHtml(req.acceptanceCriteria || '无'))]
                                        })
                                    ]
                                })
                            ]
                ...
                new Table({
                    ...
                    rows: [
                        new TableRow({
                            children: [
                                new TableCell({
                                    children: [new Paragraph('操作步骤')],
                                    shading: { fill: 'f5f5f5' }
                                }),
                                new TableCell({
                                    children: [new Paragraph(stripHtml(tc.steps || '无'))]
                                })
                            ]
                        }),
                        new TableRow({
                            children: [
                                new TableCell({
                                    children: [new Paragraph('预期结果')],
                                    shading: { fill: 'f5f5f5' }
                                }),
                                new TableCell({
                                    children: [new Paragraph(stripHtml(tc.expectedResult || '无'))]
                                })
                            ]
                        }),
                        new TableRow({
                            children: [
                                ...
                                new TableCell({
                                    children: [new Paragraph(tc.status || '-')]
                                })
                            ]
                        }),
                        new TableRow({
                            children: [
                                ...
                                new TableCell({
                                    children: [new Paragraph(tc.priority || '-')]
                                })
                            ]
                        })
                    ]
    ...
    // 追踪矩阵章节
    if (includeTraceMatrix && traces.length > 0) {
        children.push(
            new Paragraph({
                text: '3. 需求 - 用例追踪矩阵',
                heading: HeadingLevel.HEADING_1,
        ...
        traces.forEach(trace => {
            const req = requirements.find(r => r.id === trace.requirementId);
            const tc = testcases.find(t => t.id === trace.testcaseId);
            if (req && tc) {
                const key = req.reqId;
                if (!traceMap.has(key)) {
                    traceMap.set(key, { req, testcases: [] });
                }
                traceMap.get(key).testcases.push(tc);
            }
        });
        ...
        const tableRows = [
            new TableRow({
                children: [
                    new TableCell({
                        children: [new Paragraph({ text: '需求 ID', alignment: AlignmentType.CENTER })],
                        shading: { fill: '667eea' },
                        verticalAlign: 'center'
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: '需求描述', alignment: AlignmentType.CENTER })],
                        shading: { fill: '667eea' },
                        verticalAlign: 'center'
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: '关联用例', alignment: AlignmentType.CENTER })],
                        shading: { fill: '667eea' },
                        verticalAlign: 'center'
                    }),
                    new TableCell({
                        children: [new Paragraph({ text: '追踪状态', alignment: AlignmentType.CENTER })],
                        shading: { fill: '667eea' },
                        verticalAlign: 'center'
                    })
                ]
        ...
        requirements.forEach(req => {
            const traced = traceMap.get(req.reqId);
            const tcText = traced ? traced.testcases.map(tc => tc.tcId).join(', ') : '未关联';
            const status = traced ? '✅ 已追踪' : '⚠️ 未追踪';
            
            tableRows.push(
                new TableRow({
                    children: [
                        new TableCell({
                            children: [new Paragraph(req.reqId)]
                        }),
                        new TableCell({
                            children: [new Paragraph(stripHtml(req.description || '-').substring(0, 100))]
                        }),
                        new TableCell({
                            children: [new Paragraph(tcText)]
                        }),
                        new TableCell({
        ...
        // 统计信息
        const coveredCount = traceMap.size;
        const uncoveredCount = requirements.length - coveredCount;
        const coverageRate = requirements.length > 0 ? Math.round(coveredCount / requirements.length * 100) : 0;
        
        children.push(
            new Paragraph({
                text: `覆盖率统计：${coveredCount}/${requirements.length} (${coverageRate}%)`,
                heading: HeadingLevel.HEADING_2,
                spacing: { before: 400 }
            })
        );
        ...
        if (uncoveredCount > 0) {
            children.push(
                new Paragraph({
                    text: `⚠️ 未覆盖需求：${uncoveredCount} 个`,
                    style: { color: 'e74c3c', bold: true }
                })
            );
        }
    }
    ...
    const doc = new Document({
        creator: '需求管理系统',
        title: `${project.name} - 需求规格说明书`,
        description: '自动生成的需求和用例文档',
        sections: [{
            properties: {},
            children
        }]
...
// 生成 Excel 文档（带层级缩进和分组折叠）
function generateExcelDocument(project, requirements, testcases, traces, options = {}) {
    const { includeRequirements = true, includeTestcases = true, includeTraceMatrix = true, groupByHeader = true } = options;
    ...
    if (includeRequirements && requirements.length > 0) {
        ...
        // 表头
        reqData.push(['需求 ID', '标题', '分类', '状态', '优先级', '负责人', '描述', '验收标准', '创建时间', '更新时间']);
            ...
            for (const [headerId, reqs] of headersMap) {
                ...
                reqs.forEach(req => {
                    reqData.push([
                        ...
                        stripHtml(req.acceptanceCriteria || ''),
                        new Date(req.createdAt).toLocaleString('zh-CN'),
                        new Date(req.updatedAt).toLocaleString('zh-CN')
                    ]);
                });
            }
        } else {
        ...
        XLSX.utils.book_append_sheet(workbook, reqWorksheet, '需求列表');
    }
    ...
    if (includeTestcases && testcases.length > 0) {
        ...
        // 表头
        tcData.push(['用例 ID', '类型', '状态', '优先级', '负责人', '操作步骤', '预期结果', '最后执行时间', '创建时间', '更新时间']);
        ...
        testcases.forEach(tc => {
            tcData.push([
                ...
                tc.lastExecuted ? new Date(tc.lastExecuted).toLocaleString('zh-CN') : '',
                new Date(tc.createdAt).toLocaleString('zh-CN'),
                new Date(tc.updatedAt).toLocaleString('zh-CN')
            ]);
        });
        ...
        XLSX.utils.book_append_sheet(workbook, tcWorksheet, '测试用例');
    }
    
    // 工作表 3：追踪矩阵
    if (includeTraceMatrix) {
        const traceData = [];
        
        // 表头
        traceData.push(['需求 ID', '需求描述', '分类', '状态', '关联用例 ID', '用例步骤', '追踪状态', '覆盖率']);
        ...
        requirements.forEach(req => {
            const traced = traceMap.get(req.id);
            const tcIds = traced ? traced.testcases.map(tc => tc.tcId).join(', ') : '未关联';
            const tcSteps = traced ? traced.testcases.map(tc => stripHtml(tc.steps || '')).join('; ') : '-';
            const status = traced ? '✅ 已追踪' : '⚠️ 未追踪';
            ...
            traceData.push([
                ...
                req.status || '',
                tcIds,
                tcSteps.substring(0, 200),
                status,
                traced ? '✓' : '✗'
            ]);
        });
        
        const traceWorksheet = XLSX.utils.aoa_to_sheet(traceData);
        ...
        // 添加筛选功能
        traceWorksheet['!autofilter'] = { ref: 'A1:H1' };
        
        XLSX.utils.book_append_sheet(workbook, traceWorksheet, '追踪矩阵');
    }
    ...
    const summaryData = [
        ...
        ['已覆盖需求', traces.filter(t => requirements.some(r => r.id === t.requirementId)).length],
        ['未覆盖需求', requirements.length - traces.filter(t => requirements.some(r => r.id === t.requirementId)).length],
        ['覆盖率', requirements.length > 0 ? Math.round(traces.filter(t => requirements.some(r => r.id === t.requirementId)).length / requirements.length * 100) + '%' : '0%'],
        [''],
        ['用例统计', ''],
        ['总用例数', testcases.length],
        ['已通过', testcases.filter(t => t.status === 'passed').length],
        ['未执行', testcases.filter(t => t.status === 'not_executed').length],
        ['已失败', testcases.filter(t => t.status === 'failed').length],
        ['执行率', testcases.length > 0 ? Math.round((testcases.filter(t => t.status !== 'not_executed').length / testcases.length) * 100) + '%' : '0%']
    ];
    
    const summaryWorksheet = XLSX.utils.aoa_to_sheet(summaryData);
    summaryWorksheet['!cols'] = [{ wch: 20 }, { wch: 30 }];
    XLSX.utils.book_append_sheet(workbook, summaryWorksheet, '统计摘要');
...
// 生成自定义模板的导出文档
async function generateCustomTemplateDocument(project, requirements, testcases, traces, template) {
    const { sections = [], format = 'word' } = template;
    ...
    if (format === 'word') {
        ...
        sections.forEach(section => {
            if (section.type === 'title') {
                children.push(
                    new Paragraph({
                        text: section.content.replace('{{projectName}}', project.name),
                        heading: HeadingLevel.TITLE
                    })
                );
            ...
            } else if (section.type === 'requirements_table') {
                children.push(
                    new Paragraph({
                        text: '需求列表',
                        heading: HeadingLevel.HEADING_1
                ...
                requirements.forEach(req => {
                    rows.push(
                        new TableRow({
                            children: section.columns.map(col => {
                                let value = '';
                                if (col.field === 'reqId') value = req.reqId;
                                else if (col.field === 'description') value = stripHtml(req.description || '');
                                else if (col.field === 'status') value = req.status || '';
                                else if (col.field === 'priority') value = req.priority || '';
                                else if (col.field === 'category') value = req.category || '';
                                else if (col.field === 'assignee') value = req.assignee || '';
    ...
    // Excel 格式
    return generateExcelDocument(project, requirements, testcases, traces);
}

// ===== 导出 API 端点 =====

// 在 handleApiRequest 函数中添加导出路由
// 找到 handleApiRequest 函数并添加新的路由处理
    console.log(`
╔══════════════════════════════════════════════════════════╗
║          需求管理系统 - 服务器已启动                      ║
╠══════════════════════════════════════════════════════════╣
