...
async function generateWordDocument(project, requirements, testcases, traces, options = {}) {
            ...
            children.push(
                new Table({
                    ...
                    rows: [
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('分类')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(req.category || '-')] })
                            ]
                        }),
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('状态')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(req.status || '-')] })
                            ]
                        }),
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('优先级')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(req.priority || '-')] })
                            ]
                        }),
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('描述')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(stripHtml(req.description || '无'))] })
                            ]
                        })
                    ]
                })
            ...
            children.push(
                new Table({
                    ...
                    rows: [
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('操作步骤')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(stripHtml(tc.steps || '无'))] })
                            ]
                        }),
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('预期结果')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(stripHtml(tc.expectedResult || '无'))] })
                            ]
                        }),
                        new TableRow({
                            children: [
                                new TableCell({ children: [new Paragraph('状态')], shading: { fill: 'f5f5f5' } }),
                                new TableCell({ children: [new Paragraph(tc.status || '-')] })
                            ]
                        })
                    ]
                })
    ...
    if (includeTraceMatrix) {
        ...
        traces.forEach(trace => {
            ...
            if (req && tc) {
                if (!traceMap.has(req.reqId)) {
                    traceMap.set(req.reqId, { req, testcases: [] });
                }
                traceMap.get(req.reqId).testcases.push(tc);
            }
        });
        ...
        const tableRows = [
            new TableRow({
                children: [
                    new TableCell({ children: [new Paragraph({ text: '需求 ID', alignment: AlignmentType.CENTER })], shading: { fill: '667eea' } }),
                    new TableCell({ children: [new Paragraph({ text: '需求描述', alignment: AlignmentType.CENTER })], shading: { fill: '667eea' } }),
                    new TableCell({ children: [new Paragraph({ text: '关联用例', alignment: AlignmentType.CENTER })], shading: { fill: '667eea' } }),
                    new TableCell({ children: [new Paragraph({ text: '状态', alignment: AlignmentType.CENTER })], shading: { fill: '667eea' } })
                ]
            })
        ];
            ...
            tableRows.push(
                new TableRow({
                    children: [
                        ...
                        new TableCell({ children: [new Paragraph(stripHtml(req.description || '-').substring(0, 80))] }),
                        new TableCell({ children: [new Paragraph(tcText)] }),
...
function generateExcelDocument(project, requirements, testcases, traces, options = {}) {
    const { includeRequirements = true, includeTestcases = true, includeTraceMatrix = true } = options;
    
    const workbook = XLSX.utils.book_new();
    
    // 工作表 1：需求列表
    if (includeRequirements && requirements.length > 0) {
        const reqData = [['需求 ID', '标题', '分类', '状态', '优先级', '负责人', '描述', '验收标准', '创建时间', '更新时间']];
        ...
        const reqWorksheet = XLSX.utils.aoa_to_sheet(reqData);
        reqWorksheet['!cols'] = [{ wch: 15 }, { wch: 30 }, { wch: 15 }, { wch: 10 }, { wch: 10 }, { wch: 12 }, { wch: 50 }, { wch: 40 }, { wch: 20 }, { wch: 20 }];
        XLSX.utils.book_append_sheet(workbook, reqWorksheet, '需求列表');
    }
    
    // 工作表 2：测试用例
    if (includeTestcases && testcases.length > 0) {
        const tcData = [['用例 ID', '类型', '状态', '优先级', '操作步骤', '预期结果', '最后执行时间', '创建时间']];
        ...
        const tcWorksheet = XLSX.utils.aoa_to_sheet(tcData);
        tcWorksheet['!cols'] = [{ wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 10 }, { wch: 60 }, { wch: 50 }, { wch: 20 }, { wch: 20 }];
        tcWorksheet['!autofilter'] = { ref: 'A1:H1' };
        XLSX.utils.book_append_sheet(workbook, tcWorksheet, '测试用例');
    }
    
    // 工作表 3：追踪矩阵
    if (includeTraceMatrix) {
        const traceData = [['需求 ID', '需求描述', '关联用例', '追踪状态', '覆盖率']];
        ...
        traces.forEach(trace => {
            ...
            if (req && tc) {
                if (!traceMap.has(req.id)) {
                    traceMap.set(req.id, { req, testcases: [] });
                }
                traceMap.get(req.id).testcases.push(tc);
            }
        });
        ...
        traceWorksheet['!cols'] = [{ wch: 15 }, { wch: 50 }, { wch: 30 }, { wch: 12 }, { wch: 10 }];
        traceWorksheet['!autofilter'] = { ref: 'A1:E1' };
